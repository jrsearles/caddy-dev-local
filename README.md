...
- **Index page** — Visit `dev.local` (or `dev.localhost`) to see all registered containers. Cards show each container's icon, image, IP, and running/stopped status with human-readable relative timestamps (e.g. "since 3m ago") that tick live on the page; Compose services are grouped under collapsible project sections with up/down counts. Each domain is listed once with its ports as copyable chips (`domain:port`). A container icon next to the "Containers" section label opens Docker Desktop's Containers tab, and each Compose project section carries the same icon in its header linking to that project's view — via `docker-desktop://dashboard/apps[/<project>]`. Per-container deep linking isn't currently supported by the Docker Desktop URL scheme. A sticky header shows container counts and the page auto-refreshes when containers change. The page defaults to your system's light/dark preference and a header toggle lets you pin light or dark mode (or switch back to following the system). An expandable panel at the bottom shows the effective running Caddy config as a collapsible, syntax-highlighted JSON tree (via the CDN-hosted [json-view](https://github.com/pgrabovets/json-view) library) with expand/collapse-all controls, falling back to plain JSON if the CDN is unreachable.
