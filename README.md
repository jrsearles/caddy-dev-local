...
- **Index page** — Visit `dev.local` (or `dev.localhost`) to see all registered containers, with the following features:
  ...
  - **Domain rows** — two copy buttons per domain: one copies the full URL (`https://domain`), the other copies `host:port`; a globe icon links directly to the service in the browser
